...
class DSVTCenterHead(CenterHead):
    ...
    def __init__(
        self,
        loss_iou=dict(type="mmdet.L1Loss", reduction="mean", loss_weight=1),
        loss_reg_iou=None,
        *args,
        **kwargs,
    ):
        ...
        self.loss_iou_reg = (
            MODELS.build(loss_reg_iou) if loss_reg_iou is not None else None
        )
    ...
    def init_weights(self):
        ...
        for head in self.task_heads[0].heads:
            if head == "heatmap":
                constant_(
                    self.task_heads[0].__getattr__(head)[-1].bias,
    ...
    def _decode_all_preds(self, pred_dict, point_cloud_range=None, voxel_size=None):
        batch_size, _, H, W = pred_dict["reg"].shape

        batch_center = (
            pred_dict["reg"].permute(0, 2, 3, 1).contiguous().view(batch_size, H * W, 2)
        )  # (B, H, W, 2)
        batch_center_z = (
            pred_dict["height"]
            .permute(0, 2, 3, 1)
    ...
    def calc_iou_loss(self, iou_preds, batch_box_preds, mask, ind, gt_boxes):
        ...
        mask = mask.bool()
        selected_iou_preds = self._transpose_and_gather_feat(iou_preds, ind)[mask]

        selected_box_preds = self._transpose_and_gather_feat(batch_box_preds, ind)[mask]
        ...
        for task_id, preds_dict in enumerate(preds_dicts):
            ...
            if "iou" in preds_dict[0]:
                ...
                if self.loss_iou_reg is not None:
                    loss_dict[f"task{task_id}.loss_reg_iou"] = self.calc_iou_reg_loss(
    ...
    def predict_by_feat(
        ...
        **kwargs,
        ...
        for task_id, preds_dict in enumerate(preds_dicts):
            ...
            batch_iou = (
                (preds_dict[0]["iou"] + 1) * 0.5 if "iou" in preds_dict[0] else None
            )
            ...
            batch_reg_preds, batch_cls_preds, batch_cls_labels, batch_iou_preds = (
                [],
                [],
                [],
                [],
    ...
    def get_task_detections(
        ...
        for i, (box_preds, cls_preds, iou_preds, cls_labels) in enumerate(
            zip(batch_reg_preds, batch_cls_preds, batch_iou_preds, batch_cls_labels)
        ):
            pred_iou = torch.clamp(iou_preds, min=0, max=1.0)
            iou_rectifier = pred_iou.new_tensor(self.test_cfg["iou_rectifier"][task_id])
            cls_preds = torch.pow(cls_preds, 1 - iou_rectifier[cls_labels]) * torch.pow(
            ...
            if top_scores.shape[0] != 0:
                ...
                all_indices = torch.arange(top_labels.size(0)).to(top_labels.device)
                ...
                boxes_for_nms[:, 4] = (boxes_for_nms[:, 4] + torch.pi) % (
                    2 * torch.pi
                ) - torch.pi
