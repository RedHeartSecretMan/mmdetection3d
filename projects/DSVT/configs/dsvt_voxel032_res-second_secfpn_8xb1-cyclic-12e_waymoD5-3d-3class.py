...
custom_imports = dict(imports=["projects.DSVT.dsvt"], allow_failed_imports=False)
...
model = dict(
    ...
    map2bev=dict(
        type="PointPillarsScatter3D", output_shape=grid_size, num_bev_feats=192
    ),
    backbone=dict(
        type="ResSECOND",
        in_channels=192,
    ...
    bbox_head=dict(
        ...
        loss_reg_iou=dict(type="mmdet3d.DIoU3DLoss", reduction="mean", loss_weight=2.0),
        norm_bbox=True,
    ),
...
custom_hooks = [
    dict(
        ...
        disable_aug_list=["GlobalRotScaleTrans", "RandomFlip3D", "ObjectSample"],
    )
]
