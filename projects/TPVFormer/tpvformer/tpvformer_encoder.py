...
class TPVFormerEncoder(TransformerLayerSequence):
    ...
    def __init__(
        ...
        ref_3d_hw = self.get_reference_points(
            tpv_h, tpv_w, self.real_z, num_points_in_pillar[0]
        )
        ref_3d_zh = self.get_reference_points(
            tpv_z, tpv_h, self.real_w, num_points_in_pillar[1]
        )
        ...
        ref_3d_wz = self.get_reference_points(
            tpv_w, tpv_z, self.real_h, num_points_in_pillar[2]
        )
    ...
    def get_cross_view_ref_points(tpv_h, tpv_w, tpv_z, num_points_in_pillar):
        ...
        z_ranges = (
            torch.linspace(0.5, tpv_z - 0.5, num_points_in_pillar[2]) / tpv_z
        )  # #p
        z_ranges = z_ranges.unsqueeze(0).expand(tpv_h * tpv_w, -1)  # hw, #p
        h_ranges = torch.linspace(0.5, tpv_h - 0.5, tpv_h) / tpv_h
        h_ranges = (
            h_ranges.reshape(-1, 1, 1)
            .expand(-1, tpv_w, num_points_in_pillar[2])
            .flatten(0, 1)
        )
        ...
        w_ranges = torch.linspace(0.5, tpv_w - 0.5, tpv_w) / tpv_w
        w_ranges = (
            w_ranges.reshape(1, -1, 1)
            .expand(tpv_h, -1, num_points_in_pillar[2])
            .flatten(0, 1)
        )
        ...
        h_ranges = (
            h_ranges.reshape(1, -1, 1)
            .expand(tpv_z, -1, num_points_in_pillar[1])
            .flatten(0, 1)
        )
        ...
        z_ranges = (
            z_ranges.reshape(-1, 1, 1)
            .expand(-1, tpv_h, num_points_in_pillar[1])
            .flatten(0, 1)
        )
        h_ranges = torch.linspace(0.5, tpv_h - 0.5, tpv_h) / tpv_h
        h_ranges = (
        ...
        w_ranges = (
            w_ranges.reshape(-1, 1, 1)
            .expand(-1, tpv_z, num_points_in_pillar[0])
            .flatten(0, 1)
        )
        ...
        z_ranges = (
            z_ranges.reshape(1, -1, 1)
            .expand(tpv_w, -1, num_points_in_pillar[0])
            .flatten(0, 1)
        )
    ...
    def forward(self, mlvl_feats, batch_data_samples):
        ...
        ref_cross_view = (
            self.cross_view_ref_points.clone().unsqueeze(0).expand(bs, -1, -1, -1, -1)
        )
