...
class TPVFormerDataPreprocessor(Det3DDataPreprocessor):
    ...
    @torch.no_grad()
    def voxelize(self, points: List[Tensor], data_samples: SampleList) -> List[Tensor]:
        ...
        for point, data_sample in zip(points, data_samples):
            ...
            max_bound = point.new_tensor(self.voxel_layer.point_cloud_range[3:])
            point_clamp = torch.clamp(point, min_bound, max_bound + 1e-6)
            coors = torch.floor(
                (point_clamp - min_bound)
                / point_clamp.new_tensor(self.voxel_layer.voxel_size)
            ).int()
    ...
    def get_voxel_seg(self, res_coors: Tensor, data_sample: SampleList):
        ...
        if self.training:
            ...
            voxel_semantic_mask, voxel_coors, point2voxel_map = dynamic_scatter_3d(
                pts_semantic_mask, res_coors, "mean", True
            )
...
class GridMask(nn.Module):
    ...
    def forward(
        ...
        mask = np.array(mask)
        mask = mask[
            int(0.25 * height) : int(0.25 * height) + height,
        ...
        if self.offset:
            offset = inputs.new_tensor(2 * (np.random.rand(height, width) - 0.5))
