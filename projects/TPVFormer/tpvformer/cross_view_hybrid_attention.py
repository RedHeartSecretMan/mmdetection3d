...
class TPVCrossViewHybridAttention(BaseModule):
    ...
    def init_weights(self):
        ...
        if self.init_mode == 0:
            # num_phi = 4
            phi = torch.arange(4, dtype=torch.float32, device=device) * (
                2.0 * math.pi / 4
            )
            assert self.num_heads % 4 == 0
            num_theta = int(self.num_heads / 4)
            theta = torch.arange(num_theta, dtype=torch.float32, device=device) * (
                math.pi / num_theta
            ) + (
                math.pi / num_theta / 2
            )  # 3
            x = torch.matmul(
                theta.sin().unsqueeze(-1), phi.cos().unsqueeze(0)
            ).flatten()
            y = torch.matmul(
                theta.sin().unsqueeze(-1), phi.sin().unsqueeze(0)
            ).flatten()
        ...
        for i in range(3):
            ...
            grid = grid.reshape(
                self.num_heads, self.num_levels, self.num_anchors, -1, 2
            )
    ...
    def get_sampling_offsets_and_attention(
        ...
        for i, (query, fc, attn) in enumerate(
            ...
            offset = fc(query).reshape(
                bs, l, self.num_heads, self.num_levels, self.num_points, 2
            )
    ...
    def forward(
        ...
        results = []
        for out, layer, drop, residual in zip(
            outputs, self.output_proj, self.dropout, identity
        ):
