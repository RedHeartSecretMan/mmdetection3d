...
class TPVFormerPositionalEncoding(BaseModule):
    ...
    def __init__(
        self, num_feats, h, w, z, init_cfg=dict(type="Uniform", layer="Embedding")
    ):
    ...
    def forward(self, bs, device, ignore_axis="z"):
        if ignore_axis == "h":
            h_embed = torch.zeros(1, 1, self.num_feats[0], device=device).repeat(
                self.w, self.z, 1
            )  # w, z, d
            ...
            w_embed = torch.zeros(1, 1, self.num_feats[1], device=device).repeat(
                self.z, self.h, 1
            )
            z_embed = self.z_embed(torch.arange(self.z, device=device))
            ...
            z_embed = torch.zeros(1, 1, self.num_feats[2], device=device).repeat(
                self.h, self.w, 1
            )

        pos = (
            torch.cat((h_embed, w_embed, z_embed), dim=-1)
            .flatten(0, 1)
            .unsqueeze(0)
            .repeat(bs, 1, 1)
        )
