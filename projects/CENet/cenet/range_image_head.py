...
class RangeImageHead(Base3DDecodeHead):
    ...
    def _stack_batch_gt(self, batch_data_samples: SampleList) -> Tensor:
        gt_semantic_segs = [
            data_sample.gt_pts_seg.semantic_seg for data_sample in batch_data_samples
        ]
    ...
    def predict(
        self, inputs: Tuple[Tensor], batch_input_metas: List[dict], test_cfg: ConfigType
    ) -> torch.Tensor:
        ...
        for i in range(len(batch_input_metas)):
            ...
            unproj_range = torch.tensor(
                input_metas["unproj_range"], dtype=torch.float32, device=device
            )

            if use_knn:
                seg_label_list.append(
                    post_module(proj_range, unproj_range, seg_labels[i], proj_x, proj_y)
                )
            else:
