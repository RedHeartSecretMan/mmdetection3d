...
class KNN(nn.Module):
    ...
    def __init__(
        self, test_cfg: ConfigType, num_classes: int, ignore_index: int
    ) -> None:
    ...
    def forward(
        ...
        inv_gauss_k = (1 - get_gaussian_kernel(self.search, self.sigma)).view(1, -1, 1)
        ...
        knn_argmax_onehot = torch.zeros(
            (1, self.num_classes + 1, P[0]), device=proj_range.device
