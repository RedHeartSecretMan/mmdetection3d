...
class Projector:
    ...
    def compute_angle(self, xyz, query_camera, train_cameras):
        ...
        query_pose = query_camera[-16:].reshape(-1, 4, 4).repeat(num_views, 1, 1)
    ...
    def compute(self, xyz, train_imgs, train_cameras, featmaps=None, grid_sample=True):
        ...
        if featmaps is not None:
            if grid_sample:
                ...
                resize_factor = torch.tensor([f_w / w - 1.0, f_h / h - 1.0]).to(
                ...
                feat_sampled = torch.zeros(
                    (n_images, n_channels, sample_x.shape[-1]), device=featmaps.device
                )
                for i in range(n_images):
                    feat_sampled[i, :, valid[i]] = featmaps[
                        i, :, sample_y[i, valid[i]], sample_y[i, valid[i]]
                    ]
