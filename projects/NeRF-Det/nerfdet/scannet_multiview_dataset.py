    ...
    def parse_data_info(self, info: dict) -> dict:
        ...
        if self.modality["use_lidar"]:
            ...
            raise NotImplementedError(
                "Please modified " "`MultiViewPipeline` to support lidar processing"
            )
        ...
        for i in range(len(info["img_paths"])):
            ...
            if "depth_info" in info.keys():
                if self.modality["use_neuralrecon_depth"]:
                    info["depth_info"].append(dict(filename=img_filename[:-4] + ".npy"))
                else:
                    info["depth_info"].append(dict(filename=img_filename[:-4] + ".png"))
            # implement lidar_info in input.keys() in the future.
            extrinsic = np.linalg.inv(info["axis_align_matrix"] @ info["lidar2cam"][i])
            info["lidar2img"].append(extrinsic.astype(np.float32))
            if self.modality["use_ray"]:
                c2w = (info["axis_align_matrix"] @ info["lidar2cam"][i]).astype(
                    np.float32
                )  # noqa
                info["c2w"].append(c2w)
