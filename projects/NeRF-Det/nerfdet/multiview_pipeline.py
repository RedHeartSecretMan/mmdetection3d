    ...
    def transform(self, results: dict) -> dict:
        ...
        if self.loading == "random":
            ...
            if self.nerf_target_views != 0:
                target_id = np.random.choice(ids, self.nerf_target_views, replace=False)
        ...
        for i in ids:
            ...
            _results["img_norm_cfg"] = dict(mean=self.mean, std=self.std, to_rgb=True)
            # pad
            for key in _results.get("img_fields", ["img"]):
            ...
            if "depth_info" in results.keys():
                if ".npy" in results["depth_info"][i]["filename"]:
                    ...
                    _results["depth"] = (
                        np.asarray((Image.open(results["depth_info"][i]["filename"])))
                        / 1000
                    )
        ...
        if "ray_info" in results.keys():
            ...
            for i in target_id:
                ...
                pixelcoords = np.stack((px, py), axis=-1).astype(
                    np.float32
                )  # H x W x 2
                ...
                # pad
                for key in temp_results.get("img_fields", ["img"]):
                    padded_img = mmcv.impad(temp_results[key], shape=size, pad_val=0)
                ...
                if "depth_info" in results.keys():
                    if ".npy" in results["depth_info"][i]["filename"]:
                        ...
                        _results["depth"] = mmcv.imresize(
                            _results["depth"], (gt_rgb_shape[1], gt_rgb_shape[0])
                        )

                    _results["depth"] = _results["depth"]
                    gt_depth = _results["depth"][
                        py.astype(np.int32), px.astype(np.int32)
                    ]
...
class RandomShiftOrigin(BaseTransform):
    ...
    def transform(self, results):
        shift = np.random.normal(0.0, self.std, 3)
